global using Object = System.Infra.Object;
global using Range = System.Infra.Range;
global using RangeInfra = System.Infra.RangeInfra;
global using StringComparer = System.Infra.StringComparer;
global using List = System.List.List;
global using Map = System.List.Map;
global using ListIter = System.List.ListIter;
global using MapIter = System.List.MapIter;
global using ArrayIter = System.List.ArrayIter;
global using IIter = System.List.IIter;
global using Pair = System.List.Pair;
global using Array = System.List.Array;
global using Text = System.Text.Text;
global using TextPos = System.Text.Pos;
global using TextLine = System.Text.Line;
global using TextLineIter = System.Text.LineIter;
global using TextCharIter = System.Text.CharIter;
global using TextInfra = System.Text.Infra;
global using Source = Class.Infra.Source;
global using SourceArray = Class.Infra.SourceArray;
global using Error = Class.Infra.Error;
global using ErrorKind = Class.Infra.ErrorKind;
global using ErrorList = Class.Infra.ErrorList;
global using Data = Class.Infra.Data;
global using Convert = Class.Infra.Convert;
global using Constant = Class.Infra.Constant;
global using PortPort = Class.Port.Port;
global using PortRead = Class.Port.Read;
global using PortModuleName = Class.Port.ModuleName;
global using PortModuleVer = Class.Port.ModuleVer;
global using PortImportList = Class.Port.ImportList;
global using PortExportList = Class.Port.ExportList;
global using PortEntry = Class.Port.Entry;
global using PortImport = Class.Port.Import;
global using PortExport = Class.Port.Export;
global using TokenCompile = Class.Token.Compile;
global using Code = Class.Token.Code;
global using TokenToken = Class.Token.Token;
global using Comment = Class.Token.Comment;
global using TokenResult = Class.Token.Result;
global using TextRange = Class.Token.TextRange;
global using NodeCompile = Class.Node.Compile;
global using NodeNode = Class.Node.Node;
global using NodeResult = Class.Node.Result;
global using Tree = Class.Node.Tree;
global using TreeList = Class.Node.TreeList;
global using NodeList = Class.Node.NodeList;
global using CheckCompile = Class.Check.Compile;
global using CheckResult = Class.Check.Result;
global using CheckCheck = Class.Check.Check;
global using CheckModule = Class.Check.Module;
global using CheckClass = Class.Check.Class;
global using CheckField = Class.Check.Field;
global using CheckMethod = Class.Check.Method;
global using CheckVar = Class.Check.Var;
global using CheckClassMap = Class.Check.ClassMap;
global using CheckClassName = Class.Check.ClassName;
global using CheckModuleName = Class.Check.ModuleName;
global using CheckModuleIntent = Class.Check.ModuleIntent;
global using CheckModuleVer = Class.Check.ModuleVer;
global using CheckModuleRefer = Class.Check.ModuleRefer;
global using CheckModuleEntry = Class.Check.ModuleEntry;
global using CheckModuleEntryIntentMap = Class.Check.ModuleEntryIntentMap;
global using CheckModuleEntryNameMap = Class.Check.ModuleEntryNameMap;
global using CheckModuleReferCompare = Class.Check.ModuleReferCompare;
global using CheckImport = Class.Check.Import;
global using CheckExport = Class.Check.Export;
global using CheckImportList = Class.Check.ImportList;
global using CheckExportList = Class.Check.ExportList;
global using CheckEntry = Class.Check.Entry;
global using ModuleCompile = Class.Module.Compile;
global using ModuleResult = Class.Module.Result;
global using Console = System.Console;
global using Type = System.Type;
global using NullableULong = System.Nullable<ulong>;
global using SystemArray = System.Array;
global using IEnumerable = System.Collections.IEnumerable;
global using IEnumerator = System.Collections.IEnumerator;
global using StringBuilder = System.Text.StringBuilder;
global using PropertyInfo = System.Reflection.PropertyInfo;
global using FieldInfo = System.Reflection.FieldInfo;
global using BindingFlags = System.Reflection.BindingFlags;
global using PropertyInfoDictionary = System.Collections.Generic.Dictionary<string, System.Reflection.PropertyInfo>;
global using IEnumerablePropertyInfo = System.Collections.Generic.IEnumerable<System.Reflection.PropertyInfo>;
global using FieldInfoDictionary = System.Collections.Generic.Dictionary<string, System.Reflection.FieldInfo>;
global using IEnumerableFieldInfo = System.Collections.Generic.IEnumerable<System.Reflection.FieldInfo>;
global using TextWriter = System.IO.TextWriter;
global using Path = System.IO.Path;
global using Directory = System.IO.Directory;
global using File = System.IO.File;
global using FileStream = System.IO.FileStream;